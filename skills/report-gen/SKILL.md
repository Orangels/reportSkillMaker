...
使用 Agent 工具：
  ...
  prompt: "你是数据提取和计算专家，负责根据模板分析结果提取完整的多维度数据。
    ...
    ## 执行纪律（最高优先级）
    - 读取指导文档后，必须使用文档'进度追踪'章节中预定义的 TodoWrite 模板（de1-de9，共 9 步）
    - 禁止自行精简、合并或重新组织步骤 — 9 步一步不能少
    - 禁止跳过任何步骤，特别是：
      ...
      * de7（主动发现）— 逐列扫描未利用字段、维度对称补齐、交叉分析发现，补充提取
      * de8（验证）— 必须输出清单对照表 + 主动发现汇总，逐项标注提取状态，不可跳过"
```
...
4. **分析对象合理性**：检查 extracted_data.json 中的分析对象选择是否与实际数据趋势一致（选择记录中的对象应能在第一层数据中找到对应的趋势依据）
5. 如有问题，要求相应 subagent 修正
...
3. **语义验证**（读取 JSON 文件，检查数据层级完整性）：
   ...
   - [ ] **分析对象选择记录**：包含选择结果（对象名 + 选择理由 + 计划分析维度）
