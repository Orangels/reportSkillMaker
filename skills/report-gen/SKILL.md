---
name: report-gen
description: 根据 DOCX 模板和数据文件智能生成报告。触发词：生成报告、写报告、出报告、做报告、仿写报告、生成xx报告。收到报告生成请求时必须直接用 Skill 工具调用本 skill，禁止先读取文件或做其他操作。
...
| `report_scope` | 报告限定条件及全部分析要求（自然语言） | 简单：`2025年8月` / `2025年 XX部门`<br>复杂：`2025年10月，按[分类方案]统计，重点关注[类型A/类型B/类型C]，额外关注[特殊维度]` |

**自动发现逻辑**：
- 如果用户未指定文件路径，扫描当前目录的 `.docx` 和 `.xlsx` 文件
- 如果只有一个 `.docx` 文件，自动作为模板
- 如果只有一个 `.xlsx` 文件，自动作为数据源
- `report_scope` 必须询问用户确认，**必须包含用户提出的所有分析要求**（时间/地域范围、分类方案、关注维度、深度分析重点等），整体作为自然语言写入，由 DE 和 Planner 自行处理——无需读取 subagent 指导文档来判断能否支持
...
1. 运行调度检查脚本：
```bash
python3 ${CLAUDE_SKILL_DIR}/guides/scripts/coder/check_sections.py [SESSION_DIR]
```
2. 判断脚本输出：
...
**循环结束条件**：脚本输出 `ALL_DONE` 即表示所有 section 已完成，进入步骤6b-3。
...
6. **禁止读取 subagent 指导文档** - `guides/` 下的文档由各 subagent 自己读取；遇到不确定的用户需求，直接写入 `report_scope` 向下传递
...
TodoWrite([
  ...
  { id: "step6b_sections", content: "【步骤6b-2】Writer-Coder-Section × N → 循环分批执行：每轮运行 check_sections.py [SESSION_DIR]，输出 ALL_DONE 则退出循环，输出 REMAINING=N, NEXT_BATCH=[...] 则对 batch 中的 section 并行调用 Agent，等本批完成后继续下一轮；脚本输出 ALL_DONE 才可标记 completed", status: "pending" },
