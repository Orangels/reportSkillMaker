...
本文档末尾"进度追踪"章节定义了 **wr1-wr3b 共 4 个步骤**的 TodoWrite 模板。
**你必须原样使用这 4 个预定义步骤，禁止自行精简、合并或重新组织。**
...
**第一步：划定 section 边界**
- **报告头必须作为第 0 个 section**，id 固定为 `ch0_header`，排在所有正文 section 之前。报告头的具体内容由 TA 的格式规范和结构框架决定（如有红头文字、分隔线、主标题等元素，均归入此 section；若模板无独立报告头，则记录"无报告头"并跳过生成）
...
> ⚠️ 字号必须填 **pt 单位**的值，从 TA 格式表的「字号(pt)」列取，禁止填「字号(half-pt)」列的值。

| 内容类型 | 字体 | 字号(pt) | 颜色 | 加粗 | 对齐 | 行距(twips) | 首行缩进(twips) |
|---------|------|---------|------|------|------|------------|----------------|
| [从 TA 逐项提取] | | | | | | | |
...
> **⚠️ 报告头必须作为第一个条目**，排在所有正文章节之前（若模板无独立报告头则标注"无"并跳过）。
...
> 分类和数量不限，根据模板实际写作模式自由提炼，每条给出具体可执行的句式模板。
> **必须包含数字规范条目。**

1. [识别到的写作模式名称]："[句式模板]"
2. [识别到的写作模式名称]："[句式模板]"
3. ...
N. 数字规范：[从模板分析中提取的数字格式规则]
...
TodoWrite([
  { id: "wr1", content: "【加载：步骤1】阅读模板分析文件 → 提取格式规范中字号/颜色/加粗/字体的实际数值（字号必须取「字号(pt)」列的值，禁止取「字号(half-pt)」列），记录段内加粗模式，理解结构框架逻辑", status: "pending" },
