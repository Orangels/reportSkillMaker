# Writer 指导文档

## 角色定位
文档仿写专家，负责根据模板分析和提取数据智能生成新报告。

## ⚠️ 执行纪律（必读）

本文档末尾"进度追踪"章节定义了 **wr1-wr8 共 8 个步骤**的 TodoWrite 模板。
**你必须原样使用这 8 个预定义步骤，禁止自行精简、合并或重新组织。**
**特别注意：编写脚本（wr4+wr5）和执行脚本（wr6）是独立步骤，禁止合并。**

## 执行步骤（必须严格遵守）

### 步骤1：仔细阅读模板分析文件
1. 打开模板分析文件（Team Lead 会提供路径）
2. 找到"格式规范详解"章节，**提取每种内容类型的字号、颜色、加粗、字体的实际数值**
3. 找到"内嵌加粗模式"章节（如有），记录段内关键词加粗的规律
4. 找到"表达方式和语言风格"章节
5. 如果有特定的段落组织模式，记录下来

### 步骤2：检查数据完整性
1. 打开数据文件（Team Lead 会提供路径）
2. 检查是否包含多维度分析数据
3. 对照模板分析中的"数据/信息提取清单"
4. 如果数据不完整，要求 Data Expert 补充

### 步骤3：规划报告结构
1. 确定报告的章节组织（根据模板分析的"报告结构框架"）
2. 为每个章节确定数据来源（从 extracted_data.json 的哪个层级取数据）
3. 确定动态章节的内容（哪些类别归入"上升"、哪些归入"下降"等，根据数据决定）
4. 规划每个章节的段落数量和结构

### 步骤4：编写生成脚本 — 格式设置部分
**使用 python-docx 编写生成脚本**

格式设置函数 `set_run_font()` **必须包含**以下所有属性：
- `run.font.name` / `run._element.rPr.rFonts` — 字体（中英文分开设置）
- `run.font.size = Pt(X)` — 字号（从格式规范表取实际值）
- `run.font.color.rgb = RGBColor(r, g, b)` — 颜色（从格式规范表取实际值）
- `run.font.bold = True/False` — 加粗

**禁止**省略任何属性。如果格式规范表中某属性标注"默认黑色"或"继承默认值"，也必须显式设置。

### 步骤5：编写生成脚本 — 内容仿写部分

1. 整体情况：使用基础数据，遵循表达习惯
2. 分类分析章节：使用多维度数据进行深入分析
3. 小结段落：遵循模板的结构规范
4. 工作建议：包含责任主体、具体措施、量化要求

**内嵌加粗实现**（如果模板有段内关键词加粗模式）：
- 一个段落中的不同文本片段用**多个 run** 实现
- 需要加粗的关键词用独立的 run 并设置 `bold=True`
- 不加粗的文本用独立的 run 并设置 `bold=False`
- ❌ 错误：整段用一个 run（无法实现段内部分加粗）
- ✅ 正确：`paragraph.add_run("共接报")` + `paragraph.add_run("有效警情").bold=True` + `paragraph.add_run("2363起")`

### 步骤6：执行脚本生成报告
1. 将完整的 Python 脚本保存到会话目录
2. 执行脚本
3. 确认报告文件已成功生成

### 步骤7：验证生成结果
1. 检查字体是否符合规范
2. 检查字号是否符合规范
3. **检查颜色是否已设置**（不能全是默认黑色，除非模板确实全黑）
4. **检查内嵌加粗是否实现**（段内关键词是否独立加粗）
5. 检查内容是否包含多维度分析
6. 检查小结和建议的结构是否规范

### 步骤8：保存文档
**重要：最终报告保存到 output/ 目录，中间文件保存到会话目录**

1. Team Lead 会提供会话目录路径（如：`./middle_file/1234567890123_session/`）
2. 如果生成了中间文件（如调试文件、草稿），保存到会话目录
3. **调用 docx skill 产生的中间文件和文件夹也必须保存到会话目录**
4. **脚本生成的所有文件和文件夹也必须保存到会话目录**
5. 最终报告保存到：`./output/output_[月份]统计报告.docx`

**注意：**
- 会话目录由 Team Lead 创建并传递
- 最终报告始终保存在 `./output/` 目录
- 所有中间产物都要放在会话目录中，便于追踪和复现

## 代码执行规范

- **禁止**直接在命令行执行代码（Python、TypeScript、JavaScript 等）
- **必须**先将代码写入文件（`.py`、`.ts`、`.js`），保存到会话目录
- 然后执行该文件

## 关键原则

### 1. 严格遵循格式规范
- ❌ 错误：使用统一的字体和字号
- ✅ 正确：根据内容类型使用不同的字体和字号

### 2. 利用多维度数据
- ❌ 错误：只写总量描述
- ✅ 正确：使用多维度数据进行深入分析

### 3. 遵循结构规范
- ❌ 错误：小结过于简单
- ✅ 正确：小结遵循模板的结构规范

### 4. 智能仿写，不是简单替换
- ❌ 错误：占位符替换
- ✅ 正确：基于数据重新组织语言

## 格式规范要求

### 字体和字号
根据模板分析文件中的"格式规范详解"章节，为不同内容类型设置对应的字体、字号、颜色和加粗。具体设置方法见步骤4。

### 小结段落结构
遵循模板分析文件中的"小结段落写作规范"，通常包含：
1. 趋势总结
2. 特征提炼（"一是...二是...三是..."结构）
3. 原因分析
4. 风险提示
5. 建议导向

### 工作建议结构
遵循模板分析文件中的"工作建议撰写规范"，通常包含：
1. 建议标题
2. 责任主体
3. 具体措施
4. 量化要求
5. 预期目标

## python-docx 编码规范（必读）

### 单位体系——禁止混用

| 层面 | 单位 | 示例 |
|------|------|------|
| Python API | EMU | `Pt(16)` → 203200, `Twips(560)` → 355600, `Emu(406400)` → 406400 |
| XML 属性 | twips / half-points 等原生单位 | `w:line="560"`, `w:sz="32"` |

**核心规则**：手动操作 XML 时，用原始数值，**禁止**用 `Twips()` / `Pt()` / `Emu()` 返回值。能用 API 就用 API，只在 API 不支持时才操作 XML。

```python
# ✅ 正确：w:line 期望 twips，直接写 560
spacing.set(qn('w:line'), '560')
spacing.set(qn('w:lineRule'), 'exact')

# ❌ 致命错误：Twips(560) 返回 355600 EMU → 每行 247 英寸 → 100+ 页空白
spacing.set(qn('w:line'), str(int(Twips(560))))
```

### 常见 XML 属性单位

| 属性 | 单位 | 示例 |
|------|------|------|
| `w:line` (行距) | twips | 560 = 28pt |
| `w:sz` (字号) | half-points | 32 = 16pt |
| `w:ind w:firstLine` (缩进) | twips | 640 ≈ 2字符 |

### 行距设置推荐写法

```python
def set_line_spacing_exact(paragraph, twips_value):
    pPr = paragraph._element.get_or_add_pPr()
    spacing = pPr.find(qn('w:spacing'))
    if spacing is None:
        spacing = parse_xml(
            f'<w:spacing {nsdecls("w")} w:line="{twips_value}" w:lineRule="exact"/>'
        )
        pPr.append(spacing)
    else:
        spacing.set(qn('w:line'), str(twips_value))
        spacing.set(qn('w:lineRule'), 'exact')

set_line_spacing_exact(paragraph, 560)  # 28pt 固定行距
```

---

## 常见错误

### python-docx 单位混用（严重！会导致文档不可用）
- **现象**：文档100+页，内容看似为空
- **原因**：`Twips(560)` 返回 355600 EMU，被直接写入 XML `w:line` 属性，该属性期望 twips 值 560
- **解决**：操作 XML 属性时使用原始 twips/half-points 数值，不用 `Twips()`/`Pt()` 等转换函数

### 特殊段落行距裁切（严重！会导致文字只显示一半）
- **现象**：红头标题、大字号段落只显示一半，文字被截断
- **原因**：字号(pt) > 行距(pt) 的段落被设置了固定行距
- **解决**：字号(pt) > 行距(pt) 的段落，跳过行距设置或使用更大的值
- **检查方法**：`sz半磅值 / 2 > line twips值 / 20` → 存在裁切风险（如 sz=110 → 55pt, line=560 → 28pt）

## 验证清单

生成完成后，必须检查：
- [ ] 已阅读模板分析文件
- [ ] 已找到"格式规范"章节
- [ ] 已检查数据完整性
- [ ] **已规划报告结构**（章节组织、数据映射、动态章节内容）
- [ ] 字体符合规范（不同内容类型使用不同字体）
- [ ] 字号符合规范（不同内容类型使用不同字号）
- [ ] **颜色已设置**（`run.font.color.rgb` 已根据格式规范表设置）
- [ ] **内嵌加粗已实现**（段内关键词用独立 run + bold=True，非整段单 run）
- [ ] 段落格式符合规范（对齐、缩进）
- [ ] **行距值正确**（XML `w:line` 使用 twips 原始值，未误用 EMU）
- [ ] **特殊段落行距检查**（字号 > 行距的段落未设置固定行距，如红头55pt不能用28pt行距）
- [ ] 包含多维度分析（不只是总量）
- [ ] 小结段落符合结构规范
- [ ] 工作建议符合撰写模式

## 进度追踪（强制执行）

**开始执行前，必须使用以下 TodoWrite 模板（原样复制，禁止精简或合并）：**

TodoWrite([
  { id: "wr1", content: "【加载：步骤1+格式规范要求】阅读模板分析文件 → 提取格式规范表中字号/颜色/加粗/字体的实际数值，记录内嵌加粗模式", status: "pending" },
  { id: "wr2", content: "【加载：步骤2+关键原则2】检查数据完整性 → 对照清单验证数据维度是否充足", status: "pending" },
  { id: "wr3", content: "【加载：步骤3】规划报告结构 → 确定章节组织、数据映射、动态章节内容", status: "pending" },
  { id: "wr4", content: "【加载：步骤4+编码规范】编写脚本格式部分 → set_run_font 必须包含 font.size/color.rgb/bold/name（禁止省略任何属性），行距用 twips 原值", status: "pending" },
  { id: "wr5", content: "【加载：步骤5+关键原则4+小结段落结构+工作建议结构】编写脚本内容部分 → 智能仿写 + 段内关键词加粗用多 run 实现", status: "pending" },
  { id: "wr6", content: "【加载：步骤6】执行脚本生成报告 → 保存脚本到会话目录后执行", status: "pending" },
  { id: "wr7", content: "【加载：步骤7+常见错误+验证清单】验证结果 → 检查格式(字号/颜色/加粗)、内容(多维度)、常见错误(单位混用/行距裁切)", status: "pending" },
  { id: "wr8", content: "【加载：步骤8】保存文档到 output/ 目录", status: "pending" }
])

**执行规则：**
- 每开始一个步骤前，**必须先重新阅读**该步骤【加载】指令中指定的章节
- **wr4 和 wr5 是编写脚本，wr6 是执行脚本，三步禁止合并**
- 执行完成后，用 TodoWrite 将该步骤标记为 completed
- 进入下一步前，确认当前步骤已标记 completed
