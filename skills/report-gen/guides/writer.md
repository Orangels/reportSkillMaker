...
TodoWrite([
  ...
  { id: "wr4", content: "【加载：步骤4+编码规范】编写脚本格式部分 → set_run_font 必须包含 font.size/color.rgb/bold/name（禁止省略任何属性），行距用 twips 原值", status: "pending" },
  { id: "wr5", content: "【加载：步骤5+关键原则4】编写脚本内容部分 → 按模板结构逐章节仿写 + 段内关键词加粗用多 run 实现", status: "pending" },
